...
function Footer() {
  return (
    <footer className="border-t border-border px-6 py-2 text-[10px] text-muted-foreground">
      FRIDAY AI &middot; Built with Next.js 14 + FastAPI
    </footer>
  );
}

export default function Dashboard() {
  // Activate WebSocket + health poller
  useEventSocket();
  React.useEffect(() => {
    startHealthPoller();
  }, []);
  ...
  return (
    <div className={`${inter.className} flex min-h-screen flex-col bg-background bg-grid text-foreground`}>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        >
          {/* Left column: status + controls + vision */}
